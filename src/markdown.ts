...
export function Dedent(templ: string | TemplateStringsArray, ...args: unknown[]): string {
  return (
    tsdedent(templ, ...args)
      .trim()
...
export const CodeBlock = (props: string | { content: string; language?: string }): string => {
  const delimiter = '```';
...
export const Link = ({ text, href }: { text: string; href: string }) => `<a href="${href}">${text}</a>`;

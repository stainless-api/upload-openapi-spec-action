...
type DiagnosticLevel = Stainless.Builds.Diagnostics.DiagnosticListResponse['level'];
    ...
    switch (category) {
      case 'failure': {
        switch (head.commit.completed?.conclusion) {
          case 'fatal':
            return MD.Italic('Code was not generated because there was a fatal error.');
          case 'timed_out':
      ...
      case 'merge_conflict':
        return [
          head.commit.completed?.conclusion === 'upstream_merge_conflict' ?
            MD.Italic(`There was an upstream conflict which is preventing the preview of your change.`)
          : MD.Italic(`There was a conflict between your custom code and your generated changes.`),
  ...
  return MD.Details({
    summary: [
      ...
      [
        ...
        category === 'merge_conflict' ? MD.Link({ text: 'conflict', href: '#' }) : null,
      ]
...
function StatusStep(outcome: Outcomes[string], step: 'generate' | 'lint' | 'test' | 'build'): string | null {
  if (step === 'generate') {
    let status: string;
  ...
  if (stepData.status === 'completed') {
    const status =
      stepData.completed.conclusion === 'success' ? MD.Symbol.WhiteCheckMark : MD.Symbol.Exclamation;
    const text = MD.CodeInline(`${step} ${status}`);
...
type OutcomeCategory = 'failure' | 'merge_conflict' | 'regression' | 'success' | 'pending';
...
function CompareLink(base: Outcomes[string], head: Outcomes[string]): string | null {
  if (!base.commit.completed?.commit || !head.commit.completed?.commit) return null;
...
function DiagnosticsDetails(head: Outcomes[string], base: Outcomes[string]): string | null {
...
function InstallationDetails(head: Outcomes[string], lang: string): string | null {
...
const COMMENT_TITLE = MD.Heading(`${MD.Symbol.HeavyAsterisk} Stainless SDK previews`);

function categorize(head: Outcomes[string], base?: Outcomes[string]): OutcomeCategory {
  ...
  if (!['error', 'warning', 'note', 'success'].includes(head.commit.completed?.conclusion || '')) {
    return 'pending';
  }
  ...
  for (const check of ['build', 'lint', 'test'] as const) {
    if (
      (!base?.[check] ||
        (base[check]?.status === 'completed' && base[check]?.completed.conclusion === 'success')) &&
      head[check] &&
  ...
  if (base?.diagnostics && head.diagnostics) {
    ...
    if (newDiagnostics.some((d) => ['fatal', 'error', 'warning'].includes(d.level))) {
      return 'regression';
    }
  }
...
export async function retrieveComment({ token }: { token: string }) {
  ...
  const { data: comments } = await client.repos.issues.comments.list(github.context.issue.number);
...
export async function upsertComment({
  ...
  const existingComment = comments.find((comment) => comment.body?.includes(firstLine));

...
function printCommitMessage({
  ...
  return MD.Dedent`
    ${MD.Symbol.SpeechBalloon} This PR updates ${MD.CodeInline(
      ...
      canEdit ? " To change the commit message, edit this comment." : ""
    }
...
function printFailures({
  ...
  return MD.Dedent`
    ${MD.Symbol.Exclamation} ${MD.Bold(
      "Failures.",
    )} See the ${studioLink} for details.
...
function printMergeConflicts({
  ...
  const mergeConflicts = Object.entries(outcomes)
    .map<[string, string] | null>(([lang, outcome]) => {
      ...
      if (outcome.commit.completed.conclusion === "upstream_merge_conflict") {
        return [
          lang,
          `The base branch has a conflict. ${MD.Link({
            text: "Link to conflict.",
            href: url,
          })}`,
        ];
      }
  ...
  return MD.Dedent`
    ${MD.Symbol.Zap} ${MD.Bold(
      "Merge conflicts.",
    )} You can resolve conflicts now; if you do, ${MD.Link({
      text: "re-run this GitHub action",
      href: runURL,
    })} to get diffs. If you merge before resolving conflicts, new conflict PRs will be created after merging.

    ${MD.List(
      mergeConflicts.map(
        ([lang, message]) => `${projectName}-${lang}: ${message}`,
      ),
    )}
  `;
}
      ...
      if (newDiagnostics.length > 0) {
        ...
        const diagnosticList = newDiagnostics
          ...
          .map((d) => {
            ...
            return `${DiagnosticIcon[d.level]} ${MD.Bold(d.code)}: ${
              d.message
            }`;
          })
